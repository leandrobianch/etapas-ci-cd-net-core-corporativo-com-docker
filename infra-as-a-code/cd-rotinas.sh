...
--service-principal -u $CLIENT_ID \
-p $TOKEN_CLIENT_ID

#2. https://docs.microsoft.com/pt-br/azure/app-service/quickstart-multi-container
echo "Criando o recurso: $RESOURCE_NAME"
az group create --subscription $SUBSCRIPTIONS_ID --name $RESOURCE_NAME --location "South Central US"

#3. https://docs.microsoft.com/pt-br/azure/app-service/quickstart-multi-container
echo "Criando o plano: $PLAN_NAME"
az appservice plan create --subscription $SUBSCRIPTIONS_ID --name $PLAN_NAME --resource-group $RESOURCE_NAME --sku F1 --is-linux
...
#az group delete --subscription $SUBSCRIPTIONS_ID --name $RESOURCE_NAME --yes
#8
